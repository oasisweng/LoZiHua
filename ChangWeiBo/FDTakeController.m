    ...
    if (buttonIndex == self.actionSheet.cancelButtonIndex) {
        ...
        self.imagePicker.sourceType = [[self.sources objectAtIndex:buttonIndex] integerValue];
		 if (self.imagePicker.sourceType == UIImagePickerControllerSourceTypeCamera) {
			 isChosenFromLibrary = NO;
		 } else {
			 isChosenFromLibrary = YES;
		 }
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
	...
	//include isChosenFromLibrary info into info
	NSMutableDictionary* modifiedInfo = [info mutableCopy];
	[modifiedInfo setObject:[NSNumber numberWithBool:isChosenFromLibrary] forKey:@"isChosenFromLibrary"];
	info = [modifiedInfo copy];
